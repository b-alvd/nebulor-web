...
export async function PUT(req, { params }) {
  ...
  const { name, image_url, sort_order, fields } = body;

  await db.execute({
    sql: 'UPDATE entries SET name=?, image_url=?, sort_order=?, updated_at=datetime(\'now\') WHERE id=?',
    args: [name, image_url || null, sort_order || 0, id],
  });
